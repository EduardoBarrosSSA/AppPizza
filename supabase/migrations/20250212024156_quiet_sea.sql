/*
  # Update delivery validation policies

  1. Changes
    - Add policies to allow drivers to validate their own deliveries
    - Add validation to ensure only assigned drivers can complete deliveries
    - Add trigger to update order status when delivery is completed
...
BEGIN
  ...
  IF (TG_OP = 'INSERT' AND NEW.status = 'completed') OR
     (TG_OP = 'UPDATE' AND NEW.status = 'completed' AND 
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    IF OLD.status = 'assigned' AND NEW.status NOT IN ('picked_up', 'cancelled') THEN
      RAISE EXCEPTION 'Invalid status transition from assigned';
    END IF;
    
    IF OLD.status = 'picked_up' AND NEW.status NOT IN ('completed', 'cancelled') THEN
      RAISE EXCEPTION 'Invalid status transition from picked_up';
    END IF;
    
    IF OLD.status = 'completed' OR OLD.status = 'cancelled' THEN
      RAISE EXCEPTION 'Cannot change status of completed or cancelled delivery';
    END IF;
  END IF;
